      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: wp(5), paddingVertical: 10, gap: 12 }}>
        {packages.map((pkg) => (
          <Pressable
            key={pkg.id}
            onPress={() => onBuy(pkg)}
            style={{ width: 140, backgroundColor: Colors.card, borderRadius: 20, padding: 20, alignItems: 'center', borderWidth: 1, borderColor: pkg.bonus ? Colors.correct.main : Colors.border }}
          >
            {pkg.bonus && (
